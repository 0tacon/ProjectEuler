...
std::map<uint32_t, uint32_t> GetPrimeFactors(T n)
{
    ...
    primes = primes_under_1000;
	for (uint32_t prime = 0; prime != primes.size(); prime++)
        if (primes[prime] > new_n)
            primes[prime] = 0;
...
template<typename T>
std::map<uint32_t, uint32_t> GetPrimeFactors(T n, std::vector<uint32_t> &primes_under_1000)
{
	uint32_t temp = static_cast<uint32_t>(n), new_n = static_cast<uint32_t>(n);
	std::vector<uint32_t> primes, prime_factors_under_1000;
	std::map<uint32_t, uint32_t> prime_factors;
